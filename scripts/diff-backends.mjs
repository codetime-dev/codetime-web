...
const VOLATILE_KEYS = new Set([
  ...
  'createdAt', 'created_at', // for response-side timestamps (user objects keep theirs but in lists may differ if ordering shifts)
])
...
async function hit(base, path, opts) {
  ...
  try {
    ...
    return { status: 0, body: null, error: String(e), ms: Date.now() - ms }
  }
}
...
;(async () => {
  ...
  let okEq = 0, okDiff = 0, statusDiff = 0, hardFail = 0
  const lines = []
  ...
  for (const ep of filtered) {
    ...
    if (py.status !== nx.status) {
      ...
      if (wantRaw) lines.push(`  py: ${truncate(py.body)}`)
      if (wantRaw) lines.push(`  nx: ${truncate(nx.body)}`)
      continue
    }
    ...
    if (ds.length === 0) { okEq += 1; lines.push(`OK    ${head}`); continue }
    okDiff += 1
    const onlyVol = ds.every(d => d.kind === 'vol')
    lines.push(`${onlyVol ? 'VOL ' : 'DIFF'}  ${head}  (${ds.length} delta)`)
    for (const d of ds.slice(0, 12)) {
      if (d.kind === 'only-py') lines.push(`  only-py ${d.path}=${truncate(d.py)}`)
      else if (d.kind === 'only-nx') lines.push(`  only-nx ${d.path}=${truncate(d.nx)}`)
      else if (d.kind === 'len') lines.push(`  len     ${d.path}  py=${d.py} nx=${d.nx}`)
      else lines.push(`  ${d.kind === 'vol' ? 'vol' : 'val'}     ${d.path}  py=${truncate(d.py)}  nx=${truncate(d.nx)}`)
    }
    if (ds.length > 12) lines.push(`  …+${ds.length - 12} more`)
  }
