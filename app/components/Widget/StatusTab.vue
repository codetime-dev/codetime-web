...
const qs = computed(() => {
  ...
  return new URLSearchParams({ uid: String(user.value.id), theme: theme.value }).toString()
})
...
<template>
  <WidgetPlanLimitNotice
    v-if="!isPro"
    :text="w?.limit.statusFree ?? 'Free plan hides the active project name. Pro shows the full status.'"
...
.seg { display: inline-flex; border: 1px solid var(--ct-border); border-radius: var(--ct-radius-lg); overflow: hidden; max-width: 18rem; }
