...
// DELETE /v3/auth/github/link — detach the GitHub identity from the
// currently signed-in user. Refuses to leave the account with zero
// providers (which would lock the user out permanently).
...
export default defineEventHandler(async (event) => {
  ...
  const outcome = await unlinkProviderIdentity(session.id, 'githubId')
  if (outcome === 'last-one') {
