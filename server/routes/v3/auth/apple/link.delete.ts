...
// DELETE /v3/auth/apple/link — detach the Sign in with Apple identity
// from the currently signed-in user.

defineRouteMeta({
  openAPI: {
    tags: ['auth'],
    summary: 'Disconnect Sign in with Apple from the signed-in user',
...
export default defineEventHandler(async (event) => {
  ...
  const outcome = await unlinkProviderIdentity(session.id, 'appleId')
  if (outcome === 'last-one') {
